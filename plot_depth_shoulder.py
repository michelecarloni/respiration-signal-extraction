...
depth_spalle = np.load('depth_data_yolo_spalla.npy')
i=0
for data in depth_data:
    ...
    i+=1
j=0
for data in depth_spalle:
    if data == 0:
        depth_spalle[j]=depth_spalle[j-1]
        
    j+=1
    
timestamps = np.load('timestamps_realsense.npy')
print(f"depth_data: {len(depth_data)}")
print(f"depth_spalle: {len(depth_spalle)}")
print(depth_spalle)
print(depth_data)
...
depth_spalle=depth_spalle[start_index:]
...
depth_data = depth_data-depth_spalle
...
plt.xlabel('Time (sec)')
plt.ylabel('Depth (cm)')
plt.title('Depth Difference (Chest - midpoint shoulders) over Time')
plt.grid(True)

plt.figure(figsize=(12, 6))
plt.plot(timestamps, depth_spalle)
plt.xlabel('Time (sec)')
plt.ylabel('Depth (cm)')
plt.title('Depth Midpoint Shoulders over Time')
plt.grid(True)
...
lowcut = 0.13  # Low cutoff frequency (Hz)
...
plt.title('Filtered Depth Difference (Chest - midpoint shoulders) over time')
plt.xlabel('Time (sec)')
plt.ylabel('Depth (cm)')
plt.grid(True)
...
plt.title('Depth Difference (Chest - Midpoint Shoulders) Frequency Spectrum (0.1 Hz - 1 Hz)')
